...
class MedSigLIPBase(nn.Module):
    ...
    => de la ~400M parametri antrenabili ajungem la ~5M.
    ...
    def _build_lora_backbone(self, model_path: str, rank: int, alpha: int, dropout: float) -> nn.Module:
        # Incarcam modelul pretrained in float32
...
class BiomarkerHeadsV5(nn.Module):
    ...
    Strategia: luam backbone-ul deja antrenat din v15 si inghetam toti parametrii lui.
    ...
    :param backbone:      Instanta MedSigLIPBase (sau MedSigLIPMultiTask) deja incarcata cu checkpoint v15.
